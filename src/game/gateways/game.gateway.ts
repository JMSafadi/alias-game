...
export class GameGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleStartGame(@MessageBody() startGameDto: StartGameDto, @ConnectedSocket() client: Socket) {
    ...
    this.startTurnTimer(updatedGame.id, updatedGame.currentTurn.teamName, updatedGame.timePerTurn)
  }

  // Method to init timer
  private startTurnTimer(gameId: string, teamName: string, timePerTurn: number) {
  ...
  async handleGuessWord(@MessageBody() guessWordDto: GuessWordDto, @ConnectedSocket() client: Socket) {
    const { gameId, teamName, guessWord } = guessWordDto
  // Verify guess attempt
    const result = await this.gameService.checkWordGuess(gameId, teamName, guessWord)
    // If correct, end turn
    if (result.correct) {
      const turnEnded = await this.gameService.endTurn(gameId)
      this.server.emit('turnEnded', { message: 'Correct word guessed! Turn ended', turn: turnEnded })
      // Jump next turn
      const nextTurn = await this.gameService.startNextTurn(gameId)
      this.server.emit('turnStarted', nextTurn)
      this.startTurnTimer(gameId, nextTurn.currentTurn.teamName, nextTurn.timePerTurn)
