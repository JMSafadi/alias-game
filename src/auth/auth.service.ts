...
export class AuthService {
    ...
    async signUp(signUpDto: SignUpDto): Promise<{ message: string }> {
        ...
        try {
            ...
            if (error.code === 11000) { //Check if the error is a duplicate key error (E11000).
                if (error.keyPattern?.username) {//Determine which field caused the error.
                    throw new BadRequestException(`The username '${username}' is already taken. Please choose another one.`);
                } else if (error.keyPattern?.email) {
                    throw new BadRequestException(`The email '${email}' is already in use. Please use another one.`);
                }
            }
    ...
    async login(loginDto: LoginDto): Promise<{ token: string; message: string }> {
        ...
        if (!user || !isPasswordMatched) {
            throw new UnauthorizedException('Invalid credentials. Please verify the inputted information.');
        }
