...
export class LobbyService {
    ...
    async getAllLobbies(): Promise<any[]> {
        ...
        const playerIds = lobbies.flatMap(lobby => lobby.players.map(player => player.userId));
        const uniqueUserIds = [...new Set([...ownerIds, ...playerIds])];

        const users = await this.userModel.find({ _id: { $in: uniqueUserIds } }, 'username').exec();
    ...
    async createLobby(createLobbyDto: CreateLobbyDto) {
        ...
        if (!user) {
            throw new NotFoundException(`User Not Found: User with ID ${createLobbyDto.userId} not found.`);
        }
    ...
    async joinLobby(joinLobbyDto: JoinLobbyDto): Promise<any> {
        ...
        if (!mongoose.Types.ObjectId.isValid(joinLobbyDto.lobbyId)) {
            throw new BadRequestException(`Invalid lobby ID: ${joinLobbyDto.lobbyId}`);
        }

        //Find the lobby by ID.
        const lobby = await this.lobbyModel.findById(joinLobbyDto.lobbyId);
        if (!lobby) {
            throw new NotFoundException(`Lobby with ID ${joinLobbyDto.lobbyId} not found.`);
        }

        //Check if the user is already in the lobby.
        const isUserAlreadyInLobby = lobby.players.some(player => player.userId === joinLobbyDto.userId);
    ...
    async assignTeams(assignTeamsDto: AssignTeamsDto): Promise<any> {
        ...
        if (!mongoose.Types.ObjectId.isValid(assignTeamsDto.lobbyId)) {
            throw new BadRequestException(`Invalid lobby ID: ${assignTeamsDto.lobbyId}.`);
        }

        //Find the lobby by its ID.
        const lobby = await this.lobbyModel.findById(assignTeamsDto.lobbyId);
        if (!lobby) {
            throw new NotFoundException(`Lobby with ID ${assignTeamsDto.lobbyId} not found.`);
        }
        ...
        const missingPlayers = allPlayers.filter(playerId => !lobbyPlayerIds.includes(playerId));
        if (missingPlayers.length > 0) {
            throw new BadRequestException('Invalid Team Assignment: Some players are not in the lobby.');
        }
        ...
        const teamsWithUsernames = assignTeamsDto.teams.map(team => ({
            ...
            players: team.players.map(playerId => userMap.get(playerId.toString()) || playerId) //Replace userId with username.
        }));
    ...
    async deleteLobbyById(lobbyId: string): Promise<{ message: string }> {
        if (!mongoose.Types.ObjectId.isValid(lobbyId)) {
            throw new BadRequestException(`Invalid lobby ID: The provided ID is not a valid format.`);
        }

        const lobby = await this.lobbyModel.findByIdAndDelete(lobbyId);

        if (!lobby) {
            throw new NotFoundException(`Lobby Not Found: Lobby with ID ${lobbyId} not found.`);
        }
