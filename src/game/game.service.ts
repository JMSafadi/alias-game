  ...
  async checkWordGuess(gameId: string, teamName: string, guessWord: string) {
    try {
      ...
      if (game.currentTurn.teamName !== teamName) {
        throw new ForbiddenException('Not your team\s turn!')
      }
      // Verify is guess word is correct
      console.log(game.currentTurn.wordToGuess.toLowerCase())
      const isCorrect = game.currentTurn.wordToGuess.toLowerCase() === guessWord.toLowerCase()
      console.log(`Word ${guessWord} is ${isCorrect}`)
      if (isCorrect) {
        // Add point to team
        const teamIndex = game.teamsInfo.findIndex(team => team.teamName === teamName)
        if (teamIndex !== -1) {
